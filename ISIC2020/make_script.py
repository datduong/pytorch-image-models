...
python3 train.py $data_path --model MODEL-NAME -b $batchsize --sched step --epochs 450 --decay-rate 0.5 --opt nadam -j 6 --warmup-lr 1e-6 --weight-decay 0 --last_layer_weight_decay 0.0001 --drop DROPOUT --drop-connect DROPOUT --model-ema --model-ema-decay 0.9999 --aa original --vflip 0.1 --remode pixel --reprob 0 --amp --lr LEARNING-RATE --classification_layer_name 'classifier' --filter_bias_and_bn --pretrained --num-classes 7 --topk 2 --output $output --weighted_cross_entropy '30.62691 19.48443 9.11282 87.08695 8.99820 1.49366 70.52816' --create_classifier_layerfc --weighted_cross_entropy_eval --scale 0.2 1.0 --img-size 450 --crop-pct 1.0
...
for script_base in script_base_array: 
  for dropout in [0.2] : # [0.1,0.3]: 
    for index, val in enumerate(LR) :
      ...
      #
      time.sleep(3)
      os.system ( 'sbatch --partition=gpu --time=2-00:00:00 --gres=gpu:p100:1 --mem=8g -c6 ' + foutname ) # k80
