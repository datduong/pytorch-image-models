...
python3 train.py $data_path --model MODEL-NAME -b $batchsize --sched cosine --epochs 450 --decay-epochs 50 --decay-rate 0.8 --opt nadam -j 6 --warmup-lr 1e-6 --weight-decay 0 --drop DROPOUT --drop-connect DROPOUT --model-ema --model-ema-decay 0.9999 --lr LEARNING-RATE --filter_bias_and_bn --pretrained --num-classes 9 --topk 2 --output $output --scale 0.1 1.0 --eval-metric loss --amp --sampler ImbalancedDatasetSampler --aa ISIC2020 --remode pixel --reprob 0.1
  ...
  for dropout in [0.3] : # [0.1,0.3]: 
    for index, val in enumerate(LR) :
      ...
      if 'ISIC2020' in base2: 
        os.system ( 'sbatch --partition=gpu --time=1-12:00:00 --gres=gpu:p100:1 --mem=8g -c6 ' + foutname ) # k80
      else: 
        os.system ( 'sbatch --partition=gpu --time=1-12:00:00 --gres=gpu:p100:1 --mem=4g -c6 ' + foutname ) # k80
